...
public partial class Rumrum_UrunKopyalama : System.Web.UI.Page
{
    ...
    protected void DuzenleBtn_Click(object sender, EventArgs e)
    {
        if (GelID != 0)
        {
            string Git_URL = "UrunKopyalamaOnay.aspx?OID=" + Ortak.Encrypt(GelID.ToString());
            Response.Redirect(Git_URL);
        }
        else
        {
            string Git_URL = "UrunKopyalamaOnay.aspx?OID=" + Ortak.Encrypt(GeliyorID);
            Response.Redirect(Git_URL);
        }
    }
}