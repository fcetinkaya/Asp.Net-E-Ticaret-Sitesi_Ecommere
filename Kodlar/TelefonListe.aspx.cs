...
public partial class TelefonListe : System.Web.UI.Page
{
    cls_TelefonListe Kat_Cls = new cls_TelefonListe();
    cls_Urunler_CokSatanlar Urun_Cls = new cls_Urunler_CokSatanlar();
    SqlConnection con = new SqlConnection();
    private static string GelenTelefonID, Secim, SeciliTelefonID, SeciliModelID;
    public int SayfaNo
    {
        ...
        if (!IsPostBack)
        {
            try
            {
                if (RouteData.Values["TlfAdi"] != null)
                {
                    ...
                    string[] GelKAt = Kat_Cls.Kategori_Telefon_BilgisiGetir(RauteTlf);
    ...
    private void KategoriyeGore_Getir_Urunleri()
    {
        ...
        DataTable dt = Kat_Cls.RouteID_GoreGetir_Urunleri(GelenTelefonID);
        if (dt.Rows.Count != 0)
        {
    ...
    {
        DataTable dt = Kat_Cls.EnCokSatanlar(GelenTelefonID);
        if (dt.Rows.Count != 0)
        {
    ...
    private void FiyatArtana_Gore_Getir_Urunleri()
    {
        ...
        DataTable dt = Kat_Cls.FiyatArtana_GoreGetir_Urunleri(GelenTelefonID);
        if (dt.Rows.Count != 0)
        {
            Sayfa.DataSource = dt.DefaultView;
            Sayfa.AllowPaging = true;
    ...
    private void FiyatAzalana_Gore_Getir_Urunleri()
    {
        ...
        UyariDiv.Visible = false;
        KategoriRepeater.DataSource = null;
        DataTable dt = Kat_Cls.FiyatAzalan_GoreGetir_Urunleri(GelenTelefonID);
        if (dt.Rows.Count != 0)
        {
            Sayfa.DataSource = dt.DefaultView;
            Sayfa.AllowPaging = true;
    ...
    private void AdanZyeGore_Getir_Urunleri()
    {
        ...
        KategoriRepeater.Visible = true;
        UyariDiv.Visible = false;
        DataTable dt = Kat_Cls.AdanZye_GoreGetir_Urunleri(GelenTelefonID);
        if (dt.Rows.Count != 0)
        {
            Sayfa.DataSource = dt.DefaultView;
            Sayfa.AllowPaging = true;
    ...
    private void ZdenAyaGore_Getir_Urunleri()
    {
        ...
        UyariDiv.Visible = false;
        Sayfa = new PagedDataSource();
        DataTable dt = Kat_Cls.ZdenAya_GoreGetir_Urunleri(GelenTelefonID);
        if (dt.Rows.Count != 0)
        {
            Sayfa.DataSource = dt.DefaultView;
            Sayfa.AllowPaging = true;
    ...
    protected void KampanyaEpostaBtn_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Session["E_ticaretim"] != null)
            {
                string Aciklama = RouteData.Values["TlfAdi"].ToString() + " Telefonu için Talep Var.";
