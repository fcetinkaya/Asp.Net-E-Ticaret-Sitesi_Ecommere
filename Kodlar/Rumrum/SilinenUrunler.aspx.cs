...
public partial class Rumrum_SilinenUrunler : System.Web.UI.Page
{
    cls_Admin_KategoriIslemleri K = new cls_Admin_KategoriIslemleri();
    cls_Admin_SilinenUrunler SU = new cls_Admin_SilinenUrunler();
    [ScriptMethod()]
    [WebMethod]
    public static List<string> SearchCustomers(string prefixText, int count)
    {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("select UrunAdi from E_Urunler where IsActive=0 and UrunAdi like '%" + prefixText + "%'", conn))
            {
        ...
        try
        {
            if (Session["eticaret"] != null)
            {
                if (!IsPostBack)
                {
                    ...
                    label.Text = "Silinen Ürünler Listesi";
                    Label label2 = Master.FindControl("TepeMesajLbl") as Label;
                    label2.Text = "Silinen Ürünler Listesi";
                    Gel_Urunler();
                    TelefonMarkaGetir();
                    KategoriGetir_Getir();
                }
    ...
    {
        DataTable dt = SU.Liste_Gel_Arama_Kriterine_Gore(_Aramaci);
        if (dt.Rows.Count != 0)
        {
            Urunler_Grid.DataSource = dt;
            Urunler_Grid.DataBind();
    ...
    protected void Urunler_Grid_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            if (SU.Urunu_Geri_Al(e.CommandArgument.ToString()) == true)
            {
                Gel_Urunler();
                Ortak.MesajGoster("Ürün Geri Alındı.");
            }
        }
    }
}