...
public partial class Rumrum_TamamlananSiparisler : System.Web.UI.Page
{
    ...
    public static List<string> SearchCustomers_siparisNo(string prefixText, int count)
    {
        using (SqlConnection conn = new SqlConnection(Yol.ECon))
        {
            using (SqlCommand cmd = new SqlCommand("select SiparisNoFiyat from E_SiparisTakip where IsActive=1 and IslemTamam=1 and SiparisNo like '%" + prefixText + "%'", conn))
            {
                //  cmd.Parameters.AddWithValue("@SearchText", prefixText);
                if (conn.State == ConnectionState.Closed)
                {
        ...
        try
        {
            if (Session["eticaret"] != null)
            {
                if (!IsPostBack)
                {
                    ...
                    label.Text = "Tamamlanan Siparişler";
                    Label label2 = Master.FindControl("TepeMesajLbl") as Label;
                    label2.Text = "Tamamlanan Sipariş Listesi";
    ...
    {
        DataTable dt = S.Liste_Gel_Tamamlanan();
        if (dt.Rows.Count != 0)
        {
    ...
    {
        DataTable dt = S.Liste_Gel_Tamamlanan_Arama_Kriterine_Gore(Aranacak);
        if (dt.Rows.Count != 0)
        {
            Urunler_Grid.DataSource = dt;
            Urunler_Grid.DataBind();
    ...
    protected void DurumChangeBtn_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (S.Siparisi_Durum_Getir(Durumlar[0]) == true)
            {
                TamamDevamCheck.Checked = true;
            }
            else
            {
