...
public partial class Rumrum_StokdaKalmayanListesi : System.Web.UI.Page
{
    ...
    public static List<string> SearchCustomers(string prefixText, int count)
    {
        ...
        {
            using (SqlCommand cmd = new SqlCommand("select UrunAdi from E_Urunler where IsActive=1 and Tukendi=1 and UrunAdi like '%" + prefixText + "%'", conn))
            {
        ...
        try
        {
            if (Session["eticaret"] != null)
            {
                if (!IsPostBack)
                {
                    ...
                    label.Text = "Stokda Kalmayan Ürünler Listesi";
                    Label label2 = Master.FindControl("TepeMesajLbl") as Label;
                    label2.Text = "Stokda Kalmayan Ürünler Listesi";
                    Gel_Urunler();
                    TelefonMarkaGetir();
                    KategoriGetir_Getir();
                }
    ...
    {
        DataTable dt = UL.Liste_Gel_Stokta_Olmayan();
        if (dt.Rows.Count != 0)
        {
    ...
    {
        DataTable dt = UL.Liste_Gel_Arama_Kriterine_Gore_Stokta_Olmayan(_Aramaci);
        if (dt.Rows.Count != 0)
        {
            Urunler_Grid.DataSource = dt;
            Urunler_Grid.DataBind();
    ...
    protected void Urunler_Grid_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            {
                if (U.Stokta_KalmayanUrun_GeriAl(e.CommandArgument.ToString()) == true)
                {
