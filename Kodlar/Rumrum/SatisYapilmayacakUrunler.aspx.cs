...
public partial class Rumrum_SatisYapilmayacakUrunler : System.Web.UI.Page
{
        ...
        try
        {
            if (Session["eticaret"] != null)
            {
                if (!IsPostBack)
                {
                    ...
                    label.Text = "Satış Yapılmayacak Ürün İşlemi";
                    Label label2 = Master.FindControl("TepeMesajLbl") as Label;
                    label2.Text = "Satış Yapılmayacak Ürün İşlemi";
                }
            }
            else
            {
    ...
    protected void DuzenleBtn_Click(object sender, EventArgs e)
    {
        try
        {
            if (U.Satis_Iptal_Onay(GelID.ToString()) == true)
            {
