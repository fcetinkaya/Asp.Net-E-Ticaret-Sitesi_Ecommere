...
public partial class KategoriModelListe : System.Web.UI.Page
{
    cls_KategoriModelListe Kat_Cls = new cls_KategoriModelListe();
    cls_Urunler_CokSatanlar Urun_Cls = new cls_Urunler_CokSatanlar();
    SqlConnection con = new SqlConnection();
    private static string GelenKatID, GelenModelID, Secim;
    public int SayfaNo
    {
        ...
        if (!IsPostBack)
        {
            try
            {
                if (RouteData.Values["KatAdi"] != null)
                {
                    ...
                    string[] GelTlfModel = Kat_Cls.TelefonModelBilgisiGetir(RouteData.Values["ModelAdi"].ToString());
                    GelenModelID = GelTlfModel[0].ToString(); // ModelID
                    CokSatanLbl.Text = GelTlfModel[2].ToString() + " / " + GelTlfModel[1].ToString() + " " + GelKAt[1].ToString().Trim();
                    KategoriAdiLbl.Text = GelTlfModel[2].ToString() + " / " + GelTlfModel[1].ToString() + " " + GelKAt[1].ToString().Trim();
    ...
    private void KategoriyeGore_Getir_Urunleri()
    {
        ...
        DataTable dt = Kat_Cls.RouteID_GoreGetir_Urunleri(GelenKatID, GelenModelID);
        if (dt.Rows.Count != 0)
        {
    ...
    {
        DataTable dt = Kat_Cls.EnCokSatanlar(GelenKatID, GelenModelID);
        if (dt.Rows.Count != 0)
        {
    ...
    private void FiyatArtana_Gore_Getir_Urunleri()
    {
        ...
        DataTable dt = Kat_Cls.FiyatArtana_GoreGetir_Urunleri(GelenKatID, GelenModelID);
        if (dt.Rows.Count != 0)
        {
            Sayfa.DataSource = dt.DefaultView;
            Sayfa.AllowPaging = true;
    ...
    private void FiyatAzalana_Gore_Getir_Urunleri()
    {
        ...
        UyariDiv.Visible = false;
        KategoriRepeater.DataSource = null;
        DataTable dt = Kat_Cls.FiyatAzalan_GoreGetir_Urunleri(GelenKatID, GelenModelID);
        if (dt.Rows.Count != 0)
        {
            Sayfa.DataSource = dt.DefaultView;
            Sayfa.AllowPaging = true;
    ...
    private void AdanZyeGore_Getir_Urunleri()
    {
        ...
        UyariDiv.Visible = false;
        KategoriRepeater.DataSource = null;
        DataTable dt = Kat_Cls.AdanZye_GoreGetir_Urunleri(GelenKatID, GelenModelID);
        if (dt.Rows.Count != 0)
        {
            Sayfa.DataSource = dt.DefaultView;
            Sayfa.AllowPaging = true;
    ...
    private void ZdenAyaGore_Getir_Urunleri()
    {
        ...
        UyariDiv.Visible = false;
        Sayfa = new PagedDataSource();
        DataTable dt = Kat_Cls.ZdenAya_GoreGetir_Urunleri(GelenKatID, GelenModelID);
        if (dt.Rows.Count != 0)
        {
            Sayfa.DataSource = dt.DefaultView;
            Sayfa.AllowPaging = true;
    ...
    protected void KampanyaEpostaBtn_Click(object sender, EventArgs e)
    {
        try
        {
            if (Session["E_ticaretim"] != null)
            {
                string Aciklama = RouteData.Values["ModelAdi"].ToString() + " / " + RouteData.Values["KatAdi"].ToString() + " Kategorisi için Talep Var.";
