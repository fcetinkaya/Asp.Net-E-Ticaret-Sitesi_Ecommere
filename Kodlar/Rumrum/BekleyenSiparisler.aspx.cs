...
public partial class Rumrum_BekleyenSiparisler : System.Web.UI.Page
{
        ...
        try
        {
            if (Session["eticaret"] != null)
            {
                if (!IsPostBack)
                {
                    ...
                    label.Text = "Bekleyen Siparişler";
                    Label label2 = Master.FindControl("TepeMesajLbl") as Label;
                    label2.Text = "Bekleyen Sipariş Listesi";
    ...
    {
        DataTable dt = S.Liste_Gel_Bekleyen();
        if (dt.Rows.Count != 0)
        {
    ...
    {
        DataTable dt = S.Liste_Gel_Bekleyen_Arama_Kriterine_Gore(Aranacak);
        if (dt.Rows.Count != 0)
        {
            Urunler_Grid.DataSource = dt;
            Urunler_Grid.DataBind();
